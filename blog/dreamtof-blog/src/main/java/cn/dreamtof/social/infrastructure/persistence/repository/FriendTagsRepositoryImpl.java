...
import cn.dreamtof.blog.social.infrastructure.persistence.po.FriendTagsPO; // PO 类
import cn.dreamtof.blog.social.domain.model.entity.FriendTags;   // 领域层 Entity
import cn.dreamtof.blog.social.infrastructure.persistence.mapper.FriendTagsMapper;
import cn.dreamtof.blog.social.domain.repository.FriendTagsRepository;
import cn.dreamtof.blog.social.application.assembler.FriendTagsAssembler; // Assembler 移至此处
...
/**
 * 友链标签表 仓储实现 (Infrastructure Layer)
 * <p>
...
public class FriendTagsRepositoryImpl extends ServiceImpl<FriendTagsMapper, FriendTagsPO> implements FriendTagsRepository {
    ...
    private final FriendTagsAssembler assembler; // 注入 MapStruct 转换器
  ...
  public PageResult<FriendTags> page(FriendTagsPageReq pageReq) {
      ...
      Page<FriendTagsPO> flexPage = Page.of(
          pageReq.getPageNum(),
          pageReq.getPageSize()
      );
    ...
    public CursorResult<FriendTags> seek(FriendTagsCursorReq req) {
        ...
        List<FriendTagsPO> poList = queryChain()
                ...
                .where(FriendTagsPO::getId).gt(lastId)
        ...
        List<FriendTagsPO> resultList = hasNext ? poList.subList(0, req.getLimit()) : poList;
