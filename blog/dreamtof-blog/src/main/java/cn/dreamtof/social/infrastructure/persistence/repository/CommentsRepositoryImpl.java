...
import cn.dreamtof.blog.social.infrastructure.persistence.po.CommentsPO; // PO 类
import cn.dreamtof.blog.social.domain.model.entity.Comments;   // 领域层 Entity
import cn.dreamtof.blog.social.infrastructure.persistence.mapper.CommentsMapper;
import cn.dreamtof.blog.social.domain.repository.CommentsRepository;
import cn.dreamtof.blog.social.application.assembler.CommentsAssembler; // Assembler 移至此处
...
public class CommentsRepositoryImpl extends ServiceImpl<CommentsMapper, CommentsPO> implements CommentsRepository {
    ...
    private final CommentsAssembler assembler; // 注入 MapStruct 转换器
  ...
  public PageResult<Comments> page(CommentsPageReq pageReq) {
      ...
      Page<CommentsPO> flexPage = Page.of(
          pageReq.getPageNum(),
          pageReq.getPageSize()
      );
    ...
    public CursorResult<Comments> seek(CommentsCursorReq req) {
        ...
        List<CommentsPO> poList = queryChain()
                ...
                .where(CommentsPO::getId).gt(lastId)
        ...
        List<CommentsPO> resultList = hasNext ? poList.subList(0, req.getLimit()) : poList;
