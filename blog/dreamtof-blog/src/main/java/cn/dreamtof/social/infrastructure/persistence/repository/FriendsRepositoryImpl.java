...
import cn.dreamtof.blog.social.infrastructure.persistence.po.FriendsPO; // PO 类
import cn.dreamtof.blog.social.domain.model.entity.Friends;   // 领域层 Entity
import cn.dreamtof.blog.social.infrastructure.persistence.mapper.FriendsMapper;
import cn.dreamtof.blog.social.domain.repository.FriendsRepository;
import cn.dreamtof.blog.social.application.assembler.FriendsAssembler; // Assembler 移至此处
...
public class FriendsRepositoryImpl extends ServiceImpl<FriendsMapper, FriendsPO> implements FriendsRepository {
    ...
    private final FriendsAssembler assembler; // 注入 MapStruct 转换器
  ...
  public PageResult<Friends> page(FriendsPageReq pageReq) {
      ...
      Page<FriendsPO> flexPage = Page.of(
          pageReq.getPageNum(),
          pageReq.getPageSize()
      );
    ...
    public CursorResult<Friends> seek(FriendsCursorReq req) {
        ...
        List<FriendsPO> poList = queryChain()
                ...
                .where(FriendsPO::getId).gt(lastId)
        ...
        List<FriendsPO> resultList = hasNext ? poList.subList(0, req.getLimit()) : poList;
