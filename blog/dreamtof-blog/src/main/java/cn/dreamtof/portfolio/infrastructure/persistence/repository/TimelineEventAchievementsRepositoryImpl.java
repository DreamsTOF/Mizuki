...
import cn.dreamtof.blog.portfolio.infrastructure.persistence.po.TimelineEventAchievementsPO; // PO 类
import cn.dreamtof.blog.portfolio.domain.model.entity.TimelineEventAchievements;   // 领域层 Entity
import cn.dreamtof.blog.portfolio.infrastructure.persistence.mapper.TimelineEventAchievementsMapper;
import cn.dreamtof.blog.portfolio.domain.repository.TimelineEventAchievementsRepository;
import cn.dreamtof.blog.portfolio.application.assembler.TimelineEventAchievementsAssembler; // Assembler 移至此处
...
/**
 * 时间线成就关联表 仓储实现 (Infrastructure Layer)
 * <p>
...
public class TimelineEventAchievementsRepositoryImpl extends ServiceImpl<TimelineEventAchievementsMapper, TimelineEventAchievementsPO> implements TimelineEventAchievementsRepository {
    ...
    private final TimelineEventAchievementsAssembler assembler; // 注入 MapStruct 转换器
   ...
   public TimelineEventAchievements create(TimelineEventAchievements entity) {
    ...
    @Override
    public TimelineEventAchievements update(TimelineEventAchievements entity) {
        // Entity -> PO
        TimelineEventAchievementsPO po = assembler.toPO(entity);
    ...
    @Override
    public TimelineEventAchievements getById(UUID id) {
        TimelineEventAchievementsPO po = super.getById(id);
        // PO -> Entity
        return assembler.toEntity(po);
    }

    @Override
    public List<TimelineEventAchievements> listAll() {
        // 查出 PO 列表
        List<TimelineEventAchievementsPO> poList = super.list();
        // PO List -> Entity List
   ...
   */
  @Override
  public PageResult<TimelineEventAchievements> page(TimelineEventAchievementsPageReq pageReq) {
      // 1. 初始化 MyBatis-Flex 的 Page 对象
      Page<TimelineEventAchievementsPO> flexPage = Page.of(
          pageReq.getPageNum(),
          pageReq.getPageSize()
      );
      ...
      // 3. 执行持久层分页查询
      Page<TimelineEventAchievementsPO> resultPage = super.page(flexPage, queryWrapper);

      // 4. 数据层级转换 (PO -> Entity)
      List<TimelineEventAchievements> entityList = assembler.toEntityList(resultPage.getRecords());
    ...
    public boolean existsById(UUID id) {
        ...
        return queryChain().where(TimelineEventAchievementsPO::getId).eq(id).exists();
    }

    @Override
    public List<TimelineEventAchievements> listByIds(List<UUID> ids) {
        List<TimelineEventAchievementsPO> poList = super.listByIds(ids);
        return assembler.toEntityList(poList);
    }

    @Override
    public CursorResult<TimelineEventAchievements> seek(TimelineEventAchievementsCursorReq req) {
        ...
        List<TimelineEventAchievementsPO> poList = queryChain()
                // 使用 gt (大于) 实现游标跳转，主键必须有序
                .where(TimelineEventAchievementsPO::getId).gt(lastId)
                .orderBy(TimelineEventAchievementsPO::getId).asc()
                .limit(req.getLimit() + 1)
                .list();

        // 3. 判断是否有下一页并截取数据
        boolean hasNext = poList.size() > req.getLimit();
        List<TimelineEventAchievementsPO> resultList = hasNext ? poList.subList(0, req.getLimit()) : poList;
