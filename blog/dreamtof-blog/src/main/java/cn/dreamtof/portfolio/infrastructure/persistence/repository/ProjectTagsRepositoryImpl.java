...
import cn.dreamtof.blog.portfolio.infrastructure.persistence.po.ProjectTagsPO; // PO 类
import cn.dreamtof.blog.portfolio.domain.model.entity.ProjectTags;   // 领域层 Entity
import cn.dreamtof.blog.portfolio.infrastructure.persistence.mapper.ProjectTagsMapper;
import cn.dreamtof.blog.portfolio.domain.repository.ProjectTagsRepository;
import cn.dreamtof.blog.portfolio.application.assembler.ProjectTagsAssembler; // Assembler 移至此处
import cn.dreamtof.blog.portfolio.api.request.ProjectTagsPageReq;
import cn.dreamtof.blog.portfolio.api.request.ProjectTagsCursorReq;
...
/**
 * 项目标签关联表 仓储实现 (Infrastructure Layer)
 * <p>
...
public class ProjectTagsRepositoryImpl extends ServiceImpl<ProjectTagsMapper, ProjectTagsPO> implements ProjectTagsRepository {
    ...
    private final ProjectTagsAssembler assembler; // 注入 MapStruct 转换器
   ...
   */
  @Override
  public PageResult<ProjectTags> page(ProjectTagsPageReq pageReq) {
      // 1. 初始化 MyBatis-Flex 的 Page 对象
      Page<ProjectTagsPO> flexPage = Page.of(
          pageReq.getPageNum(),
          pageReq.getPageSize()
      );
      ...
      // 3. 执行持久层分页查询
      Page<ProjectTagsPO> resultPage = super.page(flexPage, queryWrapper);

      // 4. 数据层级转换 (PO -> Entity)
      List<ProjectTags> entityList = assembler.toEntityList(resultPage.getRecords());
    ...
    public boolean existsById(UUID id) {
        ...
        return queryChain().where(ProjectTagsPO::getId).eq(id).exists();
    }
    ...
    @Override
    public CursorResult<ProjectTags> seek(ProjectTagsCursorReq req) {
        ...
        List<ProjectTagsPO> poList = queryChain()
                // 使用 gt (大于) 实现游标跳转，主键必须有序
                .where(ProjectTagsPO::getId).gt(lastId)
                .orderBy(ProjectTagsPO::getId).asc()
                .limit(req.getLimit() + 1)
                .list();

        // 3. 判断是否有下一页并截取数据
        boolean hasNext = poList.size() > req.getLimit();
        List<ProjectTagsPO> resultList = hasNext ? poList.subList(0, req.getLimit()) : poList;
