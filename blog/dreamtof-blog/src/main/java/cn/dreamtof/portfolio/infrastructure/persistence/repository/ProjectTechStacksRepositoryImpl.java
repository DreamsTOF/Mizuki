...
import cn.dreamtof.blog.portfolio.infrastructure.persistence.po.ProjectTechStacksPO; // PO 类
import cn.dreamtof.blog.portfolio.domain.model.entity.ProjectTechStacks;   // 领域层 Entity
import cn.dreamtof.blog.portfolio.infrastructure.persistence.mapper.ProjectTechStacksMapper;
import cn.dreamtof.blog.portfolio.domain.repository.ProjectTechStacksRepository;
import cn.dreamtof.blog.portfolio.application.assembler.ProjectTechStacksAssembler; // Assembler 移至此处
...
/**
 * 项目技术栈关联表 仓储实现 (Infrastructure Layer)
 * <p>
...
public class ProjectTechStacksRepositoryImpl extends ServiceImpl<ProjectTechStacksMapper, ProjectTechStacksPO> implements ProjectTechStacksRepository {
    ...
    private final ProjectTechStacksAssembler assembler; // 注入 MapStruct 转换器
   ...
   */
  @Override
  public PageResult<ProjectTechStacks> page(ProjectTechStacksPageReq pageReq) {
      // 1. 初始化 MyBatis-Flex 的 Page 对象
      Page<ProjectTechStacksPO> flexPage = Page.of(
          pageReq.getPageNum(),
          pageReq.getPageSize()
      );
      ...
      // 3. 执行持久层分页查询
      Page<ProjectTechStacksPO> resultPage = super.page(flexPage, queryWrapper);

      // 4. 数据层级转换 (PO -> Entity)
      List<ProjectTechStacks> entityList = assembler.toEntityList(resultPage.getRecords());
    ...
    public boolean existsById(UUID id) {
        ...
        return queryChain().where(ProjectTechStacksPO::getId).eq(id).exists();
    }
    ...
    @Override
    public CursorResult<ProjectTechStacks> seek(ProjectTechStacksCursorReq req) {
        ...
        List<ProjectTechStacksPO> poList = queryChain()
                // 使用 gt (大于) 实现游标跳转，主键必须有序
                .where(ProjectTechStacksPO::getId).gt(lastId)
                .orderBy(ProjectTechStacksPO::getId).asc()
                .limit(req.getLimit() + 1)
                .list();

        // 3. 判断是否有下一页并截取数据
        boolean hasNext = poList.size() > req.getLimit();
        List<ProjectTechStacksPO> resultList = hasNext ? poList.subList(0, req.getLimit()) : poList;
