...
import cn.dreamtof.blog.portfolio.infrastructure.persistence.po.ProjectsPO; // PO 类
import cn.dreamtof.blog.portfolio.domain.model.entity.Projects;   // 领域层 Entity
import cn.dreamtof.blog.portfolio.infrastructure.persistence.mapper.ProjectsMapper;
import cn.dreamtof.blog.portfolio.domain.repository.ProjectsRepository;
import cn.dreamtof.blog.portfolio.application.assembler.ProjectsAssembler; // Assembler 移至此处
...
public class ProjectsRepositoryImpl extends ServiceImpl<ProjectsMapper, ProjectsPO> implements ProjectsRepository {
    ...
    private final ProjectsAssembler assembler; // 注入 MapStruct 转换器
   ...
   public Projects create(Projects entity) {
    ...
    public List<Projects> listAll() {
        // 查出 PO 列表
        List<ProjectsPO> poList = super.list();
        // PO List -> Entity List
   ...
   */
  @Override
  public PageResult<Projects> page(ProjectsPageReq pageReq) {
      // 1. 初始化 MyBatis-Flex 的 Page 对象
      Page<ProjectsPO> flexPage = Page.of(
          pageReq.getPageNum(),
          pageReq.getPageSize()
      );
      ...
      // 3. 执行持久层分页查询
      Page<ProjectsPO> resultPage = super.page(flexPage, queryWrapper);
    ...
    public boolean existsById(UUID id) {
        ...
        return queryChain().where(ProjectsPO::getId).eq(id).exists();
    }
    ...
    @Override
    public CursorResult<Projects> seek(ProjectsCursorReq req) {
        ...
        List<ProjectsPO> poList = queryChain()
                // 使用 gt (大于) 实现游标跳转，主键必须有序
                .where(ProjectsPO::getId).gt(lastId)
                .orderBy(ProjectsPO::getId).asc()
                .limit(req.getLimit() + 1)
                .list();

        // 3. 判断是否有下一页并截取数据
        boolean hasNext = poList.size() > req.getLimit();
        List<ProjectsPO> resultList = hasNext ? poList.subList(0, req.getLimit()) : poList;
