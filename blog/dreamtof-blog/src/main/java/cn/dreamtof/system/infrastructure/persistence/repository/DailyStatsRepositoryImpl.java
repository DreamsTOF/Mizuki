...
import cn.dreamtof.blog.system.infrastructure.persistence.po.DailyStatsPO; // PO 类
import cn.dreamtof.blog.system.domain.model.entity.DailyStats;   // 领域层 Entity
import cn.dreamtof.blog.system.infrastructure.persistence.mapper.DailyStatsMapper;
import cn.dreamtof.blog.system.domain.repository.DailyStatsRepository;
import cn.dreamtof.blog.system.application.assembler.DailyStatsAssembler; // Assembler 移至此处
import cn.dreamtof.blog.system.api.request.DailyStatsPageReq;
import cn.dreamtof.blog.system.api.request.DailyStatsCursorReq;
...
/**
 * 每日统计汇总表 仓储实现 (Infrastructure Layer)
 * <p>
...
public class DailyStatsRepositoryImpl extends ServiceImpl<DailyStatsMapper, DailyStatsPO> implements DailyStatsRepository {
    ...
    private final DailyStatsAssembler assembler; // 注入 MapStruct 转换器
   ...
   */
  @Override
  public PageResult<DailyStats> page(DailyStatsPageReq pageReq) {
      // 1. 初始化 MyBatis-Flex 的 Page 对象
      Page<DailyStatsPO> flexPage = Page.of(
          pageReq.getPageNum(),
          pageReq.getPageSize()
      );
    ...
    @Override
    public CursorResult<DailyStats> seek(DailyStatsCursorReq req) {
        ...
        List<DailyStatsPO> poList = queryChain()
                ...
                .where(DailyStatsPO::getId).gt(lastId)
        ...
        List<DailyStatsPO> resultList = hasNext ? poList.subList(0, req.getLimit()) : poList;
