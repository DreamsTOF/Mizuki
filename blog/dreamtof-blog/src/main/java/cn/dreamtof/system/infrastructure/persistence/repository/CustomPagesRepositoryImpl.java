...
import cn.dreamtof.blog.system.infrastructure.persistence.po.CustomPagesPO; // PO 类
import cn.dreamtof.blog.system.domain.model.entity.CustomPages;   // 领域层 Entity
import cn.dreamtof.blog.system.infrastructure.persistence.mapper.CustomPagesMapper;
import cn.dreamtof.blog.system.domain.repository.CustomPagesRepository;
import cn.dreamtof.blog.system.application.assembler.CustomPagesAssembler; // Assembler 移至此处
import cn.dreamtof.blog.system.api.request.CustomPagesPageReq;
import cn.dreamtof.blog.system.api.request.CustomPagesCursorReq;
...
/**
 * 自定义页面表 仓储实现 (Infrastructure Layer)
 * <p>
...
public class CustomPagesRepositoryImpl extends ServiceImpl<CustomPagesMapper, CustomPagesPO> implements CustomPagesRepository {
    ...
    private final CustomPagesAssembler assembler; // 注入 MapStruct 转换器
   ...
   */
  @Override
  public PageResult<CustomPages> page(CustomPagesPageReq pageReq) {
      // 1. 初始化 MyBatis-Flex 的 Page 对象
      Page<CustomPagesPO> flexPage = Page.of(
          pageReq.getPageNum(),
          pageReq.getPageSize()
      );
      ...
      // 3. 执行持久层分页查询
      Page<CustomPagesPO> resultPage = super.page(flexPage, queryWrapper);
    ...
    public boolean existsById(UUID id) {
        ...
        return queryChain().where(CustomPagesPO::getId).eq(id).exists();
    }
    ...
    @Override
    public CursorResult<CustomPages> seek(CustomPagesCursorReq req) {
        ...
        List<CustomPagesPO> poList = queryChain()
                // 使用 gt (大于) 实现游标跳转，主键必须有序
                .where(CustomPagesPO::getId).gt(lastId)
                .orderBy(CustomPagesPO::getId).asc()
                .limit(req.getLimit() + 1)
                .list();

        // 3. 判断是否有下一页并截取数据
        boolean hasNext = poList.size() > req.getLimit();
        List<CustomPagesPO> resultList = hasNext ? poList.subList(0, req.getLimit()) : poList;
