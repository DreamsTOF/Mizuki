...
import cn.dreamtof.blog.system.infrastructure.persistence.po.ThemeSettingsPO; // PO 类
import cn.dreamtof.blog.system.domain.model.entity.ThemeSettings;   // 领域层 Entity
import cn.dreamtof.blog.system.infrastructure.persistence.mapper.ThemeSettingsMapper;
import cn.dreamtof.blog.system.domain.repository.ThemeSettingsRepository;
import cn.dreamtof.blog.system.application.assembler.ThemeSettingsAssembler; // Assembler 移至此处
import cn.dreamtof.blog.system.api.request.ThemeSettingsPageReq;
import cn.dreamtof.blog.system.api.request.ThemeSettingsCursorReq;
...
/**
 * 主题设置表 仓储实现 (Infrastructure Layer)
 * <p>
...
public class ThemeSettingsRepositoryImpl extends ServiceImpl<ThemeSettingsMapper, ThemeSettingsPO> implements ThemeSettingsRepository {
    ...
    private final ThemeSettingsAssembler assembler; // 注入 MapStruct 转换器
   ...
   public ThemeSettings create(ThemeSettings entity) {
    ...
    public List<ThemeSettings> listAll() {
        // 查出 PO 列表
        List<ThemeSettingsPO> poList = super.list();
        // PO List -> Entity List
   ...
   */
  @Override
  public PageResult<ThemeSettings> page(ThemeSettingsPageReq pageReq) {
      // 1. 初始化 MyBatis-Flex 的 Page 对象
      Page<ThemeSettingsPO> flexPage = Page.of(
          pageReq.getPageNum(),
          pageReq.getPageSize()
      );
      ...
      // 3. 执行持久层分页查询
      Page<ThemeSettingsPO> resultPage = super.page(flexPage, queryWrapper);

      // 4. 数据层级转换 (PO -> Entity)
      List<ThemeSettings> entityList = assembler.toEntityList(resultPage.getRecords());
    ...
    public boolean existsById(UUID id) {
        ...
        return queryChain().where(ThemeSettingsPO::getId).eq(id).exists();
    }

    @Override
    public List<ThemeSettings> listByIds(List<UUID> ids) {
        List<ThemeSettingsPO> poList = super.listByIds(ids);
        return assembler.toEntityList(poList);
    }

    @Override
    public CursorResult<ThemeSettings> seek(ThemeSettingsCursorReq req) {
        ...
        List<ThemeSettingsPO> poList = queryChain()
                // 使用 gt (大于) 实现游标跳转，主键必须有序
                .where(ThemeSettingsPO::getId).gt(lastId)
                .orderBy(ThemeSettingsPO::getId).asc()
                .limit(req.getLimit() + 1)
                .list();

        // 3. 判断是否有下一页并截取数据
        boolean hasNext = poList.size() > req.getLimit();
        List<ThemeSettingsPO> resultList = hasNext ? poList.subList(0, req.getLimit()) : poList;
