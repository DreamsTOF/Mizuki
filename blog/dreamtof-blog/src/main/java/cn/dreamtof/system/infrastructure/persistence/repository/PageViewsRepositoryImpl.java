...
import cn.dreamtof.blog.system.infrastructure.persistence.po.PageViewsPO; // PO 类
import cn.dreamtof.blog.system.domain.model.entity.PageViews;   // 领域层 Entity
import cn.dreamtof.blog.system.infrastructure.persistence.mapper.PageViewsMapper;
import cn.dreamtof.blog.system.domain.repository.PageViewsRepository;
import cn.dreamtof.blog.system.application.assembler.PageViewsAssembler; // Assembler 移至此处
import cn.dreamtof.blog.system.api.request.PageViewsPageReq;
import cn.dreamtof.blog.system.api.request.PageViewsCursorReq;
...
/**
 * 页面访问统计表 仓储实现 (Infrastructure Layer)
 * <p>
...
public class PageViewsRepositoryImpl extends ServiceImpl<PageViewsMapper, PageViewsPO> implements PageViewsRepository {
    ...
    private final PageViewsAssembler assembler; // 注入 MapStruct 转换器
   ...
   */
  @Override
  public PageResult<PageViews> page(PageViewsPageReq pageReq) {
      // 1. 初始化 MyBatis-Flex 的 Page 对象
      Page<PageViewsPO> flexPage = Page.of(
          pageReq.getPageNum(),
          pageReq.getPageSize()
      );
    ...
    @Override
    public CursorResult<PageViews> seek(PageViewsCursorReq req) {
        ...
        List<PageViewsPO> poList = queryChain()
                ...
                .where(PageViewsPO::getId).gt(lastId)
        ...
        List<PageViewsPO> resultList = hasNext ? poList.subList(0, req.getLimit()) : poList;
