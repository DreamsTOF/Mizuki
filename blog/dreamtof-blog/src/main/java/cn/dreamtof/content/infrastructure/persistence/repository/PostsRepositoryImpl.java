...
import cn.dreamtof.blog.content.infrastructure.persistence.po.PostsPO; // PO 类
import cn.dreamtof.blog.content.domain.model.entity.Posts;   // 领域层 Entity
import cn.dreamtof.blog.content.infrastructure.persistence.mapper.PostsMapper;
import cn.dreamtof.blog.content.domain.repository.PostsRepository;
import cn.dreamtof.blog.content.application.assembler.PostsAssembler; // Assembler 移至此处
import cn.dreamtof.blog.content.api.request.PostsPageReq;
import cn.dreamtof.blog.content.api.request.PostsCursorReq;
...
/**
 * 文章主表 仓储实现 (Infrastructure Layer)
 * <p>
...
public class PostsRepositoryImpl extends ServiceImpl<PostsMapper, PostsPO> implements PostsRepository {
    ...
    private final PostsAssembler assembler; // 注入 MapStruct 转换器
   ...
   */
  @Override
  public PageResult<Posts> page(PostsPageReq pageReq) {
      // 1. 初始化 MyBatis-Flex 的 Page 对象
      Page<PostsPO> flexPage = Page.of(
          pageReq.getPageNum(),
          pageReq.getPageSize()
      );
      ...
      // 3. 执行持久层分页查询
      Page<PostsPO> resultPage = super.page(flexPage, queryWrapper);
    ...
    public boolean existsById(UUID id) {
        ...
        return queryChain().where(PostsPO::getId).eq(id).exists();
    }
    ...
    @Override
    public CursorResult<Posts> seek(PostsCursorReq req) {
        ...
        List<PostsPO> poList = queryChain()
                // 使用 gt (大于) 实现游标跳转，主键必须有序
                .where(PostsPO::getId).gt(lastId)
                .orderBy(PostsPO::getId).asc()
                .limit(req.getLimit() + 1)
                .list();

        // 3. 判断是否有下一页并截取数据
        boolean hasNext = poList.size() > req.getLimit();
        List<PostsPO> resultList = hasNext ? poList.subList(0, req.getLimit()) : poList;
