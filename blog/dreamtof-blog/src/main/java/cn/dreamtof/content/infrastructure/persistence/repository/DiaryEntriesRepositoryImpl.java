...
import cn.dreamtof.blog.content.infrastructure.persistence.po.DiaryEntriesPO; // PO 类
import cn.dreamtof.blog.content.domain.model.entity.DiaryEntries;   // 领域层 Entity
import cn.dreamtof.blog.content.infrastructure.persistence.mapper.DiaryEntriesMapper;
import cn.dreamtof.blog.content.domain.repository.DiaryEntriesRepository;
import cn.dreamtof.blog.content.application.assembler.DiaryEntriesAssembler; // Assembler 移至此处
import cn.dreamtof.blog.content.api.request.DiaryEntriesPageReq;
import cn.dreamtof.blog.content.api.request.DiaryEntriesCursorReq;
...
/**
 * 日记条目表 仓储实现 (Infrastructure Layer)
 * <p>
...
public class DiaryEntriesRepositoryImpl extends ServiceImpl<DiaryEntriesMapper, DiaryEntriesPO> implements DiaryEntriesRepository {
    ...
    private final DiaryEntriesAssembler assembler; // 注入 MapStruct 转换器
   ...
   */
  @Override
  public PageResult<DiaryEntries> page(DiaryEntriesPageReq pageReq) {
      // 1. 初始化 MyBatis-Flex 的 Page 对象
      Page<DiaryEntriesPO> flexPage = Page.of(
          pageReq.getPageNum(),
          pageReq.getPageSize()
      );
      ...
      // 3. 执行持久层分页查询
      Page<DiaryEntriesPO> resultPage = super.page(flexPage, queryWrapper);
    ...
    public boolean existsById(UUID id) {
        ...
        return queryChain().where(DiaryEntriesPO::getId).eq(id).exists();
    }
    ...
    @Override
    public CursorResult<DiaryEntries> seek(DiaryEntriesCursorReq req) {
        ...
        List<DiaryEntriesPO> poList = queryChain()
                // 使用 gt (大于) 实现游标跳转，主键必须有序
                .where(DiaryEntriesPO::getId).gt(lastId)
                .orderBy(DiaryEntriesPO::getId).asc()
                .limit(req.getLimit() + 1)
                .list();

        // 3. 判断是否有下一页并截取数据
        boolean hasNext = poList.size() > req.getLimit();
        List<DiaryEntriesPO> resultList = hasNext ? poList.subList(0, req.getLimit()) : poList;
