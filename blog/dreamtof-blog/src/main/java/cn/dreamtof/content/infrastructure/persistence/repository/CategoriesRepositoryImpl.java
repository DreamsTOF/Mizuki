...
import cn.dreamtof.blog.content.infrastructure.persistence.po.CategoriesPO; // PO 类
import cn.dreamtof.blog.content.domain.model.entity.Categories;   // 领域层 Entity
import cn.dreamtof.blog.content.infrastructure.persistence.mapper.CategoriesMapper;
import cn.dreamtof.blog.content.domain.repository.CategoriesRepository;
import cn.dreamtof.blog.content.application.assembler.CategoriesAssembler; // Assembler 移至此处
import cn.dreamtof.blog.content.api.request.CategoriesPageReq;
import cn.dreamtof.blog.content.api.request.CategoriesCursorReq;
...
/**
 * 文章分类表 仓储实现 (Infrastructure Layer)
 * <p>
...
public class CategoriesRepositoryImpl extends ServiceImpl<CategoriesMapper, CategoriesPO> implements CategoriesRepository {
    ...
    private final CategoriesAssembler assembler; // 注入 MapStruct 转换器
   ...
   */
  @Override
  public PageResult<Categories> page(CategoriesPageReq pageReq) {
      // 1. 初始化 MyBatis-Flex 的 Page 对象
      Page<CategoriesPO> flexPage = Page.of(
          pageReq.getPageNum(),
          pageReq.getPageSize()
      );
      ...
      // 3. 执行持久层分页查询
      Page<CategoriesPO> resultPage = super.page(flexPage, queryWrapper);
    ...
    public boolean existsById(UUID id) {
        ...
        return queryChain().where(CategoriesPO::getId).eq(id).exists();
    }
    ...
    @Override
    public CursorResult<Categories> seek(CategoriesCursorReq req) {
        ...
        List<CategoriesPO> poList = queryChain()
                // 使用 gt (大于) 实现游标跳转，主键必须有序
                .where(CategoriesPO::getId).gt(lastId)
                .orderBy(CategoriesPO::getId).asc()
                .limit(req.getLimit() + 1)
                .list();

        // 3. 判断是否有下一页并截取数据
        boolean hasNext = poList.size() > req.getLimit();
        List<CategoriesPO> resultList = hasNext ? poList.subList(0, req.getLimit()) : poList;
