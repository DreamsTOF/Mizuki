...
import cn.dreamtof.blog.content.infrastructure.persistence.po.ArchivesPO; // PO 类
import cn.dreamtof.blog.content.domain.model.entity.Archives;   // 领域层 Entity
import cn.dreamtof.blog.content.infrastructure.persistence.mapper.ArchivesMapper;
import cn.dreamtof.blog.content.domain.repository.ArchivesRepository;
import cn.dreamtof.blog.content.application.assembler.ArchivesAssembler; // Assembler 移至此处
...
/**
 * 文章归档索引表 仓储实现 (Infrastructure Layer)
 * <p>
...
public class ArchivesRepositoryImpl extends ServiceImpl<ArchivesMapper, ArchivesPO> implements ArchivesRepository {
    ...
    private final ArchivesAssembler assembler; // 注入 MapStruct 转换器
   ...
   */
  @Override
  public PageResult<Archives> page(ArchivesPageReq pageReq) {
      // 1. 初始化 MyBatis-Flex 的 Page 对象
      Page<ArchivesPO> flexPage = Page.of(
          pageReq.getPageNum(),
          pageReq.getPageSize()
      );
      ...
      // 3. 执行持久层分页查询
      Page<ArchivesPO> resultPage = super.page(flexPage, queryWrapper);
    ...
    public boolean existsById(UUID id) {
        ...
        return queryChain().where(ArchivesPO::getId).eq(id).exists();
    }
    ...
    @Override
    public CursorResult<Archives> seek(ArchivesCursorReq req) {
        ...
        List<ArchivesPO> poList = queryChain()
                // 使用 gt (大于) 实现游标跳转，主键必须有序
                .where(ArchivesPO::getId).gt(lastId)
                .orderBy(ArchivesPO::getId).asc()
                .limit(req.getLimit() + 1)
                .list();

        // 3. 判断是否有下一页并截取数据
        boolean hasNext = poList.size() > req.getLimit();
        List<ArchivesPO> resultList = hasNext ? poList.subList(0, req.getLimit()) : poList;
